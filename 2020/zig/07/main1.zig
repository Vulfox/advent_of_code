...
const Bag = struct {
    ...
    pub fn init(allocator: *Allocator, name: []const u8) Self {
        return Self{
            ...
            .contentsOf = ArrayList(*Bag).init(allocator),
        };
    }
};
...
pub fn main() !void {
    ...
    while(input_iter.next()) |raw_line| {
        ...
        while(bag_contents_iter.next()) |bag_content| {
            ...
            try current_content_bag.contentsOf.append(current_bag);
    ...
    var valid_parents = ArrayList(*Bag).init(allocator);
    ...
    var childCount = getChildCount(shiny_gold_bag, &known_bags, 1);
    std.debug.print("Bag Count: {}\n", .{childCount});
}

fn printBags(bags: *ArrayList(Bag)) void {
    for(bags.items) |bag| {
        std.debug.print("bag:\n  name: '{}'\n  isTopLevel: {}\n  pointer: {*}\n", .{bag.name, bag.isTopLevel, &bag});
        ...
        for(bag.contentsOf.items) |content_of_bags| {
            std.debug.print("    - name: {}\n", .{content_of_bags.name});
        }
    }
}

fn getValidParents(bag: *Bag, valid_parents: *ArrayList(*Bag), known_bags: *ArrayList(Bag), n: u32) void {
    ...
    for(bag.contentsOf.items) |parent| {
        ...
        if (!doesBagExist(valid_parents, parent)) {
            ...
            var ptr_parent = getKnownBag(known_bags, parent.name).?;
...
fn doesBagExist(valid_parents: *ArrayList(*Bag), bag: *Bag) bool {
    for(valid_parents.items) |parent_bag, _| {
        //if (parent_bag == bag) return true;
        if(std.mem.eql(u8, parent_bag.name, bag.name)) return true;
    }
